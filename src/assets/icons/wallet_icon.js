import React from 'react';

const WalletIcon = () => {
    return (
        <>
            <svg xmlns="http://www.w3.org/2000/svg" width="22.735" height="22.735" viewBox="0 0 22.735 22.735">
                <g id="icons8_wallet" transform="translate(0.5 0.5)">
                    <path id="Path_5786" dataNama="Path 5786"
                          d="M21.112,24.612H6.622A4.127,4.127,0,0,1,2.5,20.49V5.5H21.112a4.127,4.127,0,0,1,4.122,4.122V20.49A4.127,4.127,0,0,1,21.112,24.612ZM3.5,6.5V20.49a3.126,3.126,0,0,0,3.122,3.122h14.49a3.126,3.126,0,0,0,3.122-3.122V9.622A3.126,3.126,0,0,0,21.112,6.5Z"
                          transform="translate(-3 -2.378)" fill="#fff"/>
                    <path id="Ellipse_28" dataNama="Ellipse 28"
                          d="M2-.5A2.5,2.5,0,1,1-.5,2,2.5,2.5,0,0,1,2-.5Zm0,4A1.5,1.5,0,1,0,.5,2,1.5,1.5,0,0,0,2,3.5Z"
                          transform="translate(15.69 10.834)" fill="#fff"/>
                    <path id="Path_5787" dataNama="Path 5787"
                          d="M21.612,7.122H2.5v-.5A4.127,4.127,0,0,1,6.622,2.5H17.49a4.127,4.127,0,0,1,4.122,4.122Zm-18.072-1H20.572A3.127,3.127,0,0,0,17.49,3.5H6.622A3.127,3.127,0,0,0,3.54,6.122Z"
                          transform="translate(-3 -3)" fill="#fff"/>
                </g>
            </svg>
        </>
    );
};

export default WalletIcon;